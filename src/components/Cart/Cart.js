...
const Cart = ({ setCartOpen, cartOpen }) => {
    ...
    return (
        <section className={`cart top-0 z-50 h-[100vh] bg-[#CEEDC7] lg:w-1/4 w-3/4 p-6 ${cartOpen && 'open'}`} >
            <label htmlFor="my-modal-3" className="btn btn-sm btn-circle btn-error text-white absolute right-2 top-2" onClick={() => setCartOpen(false)}>✕</label>
            ...
            <h2 className='mt-4'>Total &emsp; <span className='text-3xl font-bold'>BDT {total}</span> </h2>
            {
                cartData.length && <button className='btn btn-white w-full mt-8 text-white normal-case'>
                    Checkout &emsp;
                    <lord-icon
